...
function dedupeProvs(provs){ if(!Array.isArray(provs)) return []; const by={}; const out=[]; for(const p of provs){ const nm=cleanName(p&&p.name); if(!nm) continue; const k=nm.toLowerCase(); if(by[k]){ const keep=by[k]; for(const f in p){ if((keep[f]==null||keep[f]===""||keep[f]===0) && p[f]!=null && p[f]!=="" && p[f]!==0) keep[f]=p[f]; } } else { p.name=nm; by[k]=p; out.push(p); } } return out; }
...
function writeState(obj){ backupState(); fs.writeFileSync(STATE_FILE, JSON.stringify(obj)); }
